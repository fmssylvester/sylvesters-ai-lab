...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  footer{margin-top:30px;text-align:center;color:#7e93a8;font-size:12px;letter-spacing:0.5px;}
  footer b{color:#cfe9f7;font-family:'Space Grotesk',sans-serif;}
</style>
</head>
<body>
<div class="wrap">
  <div class="header">
    ...
    <div>
      <div class="title">SYLVESTER'S <span class="accent">AI LAB</span></div>
      <div class="tag">CHANNEL DROPS · ASSET LIBRARY</div>
    </div>
  </div>
  ...
  <table>
    <thead><tr><th>Asset</th><th>Size</th><th>Added</th><th></th></tr></thead>
  ...
  <footer>SYLVESTER'S AI LAB · <b>Crafted with light</b></footer>
</div>
<script>
  const all = __DATA__;
  ...
  function render(list){
    tbody.innerHTML = list.map(it=>{
      const enc = encodeURIComponent(it.key);
      return '<tr><td class="name"><a href="/d/'+enc+'">'+esc(it.fname)+'</a></td><td>'+it.size+'</td><td>'+it.mod+'</td><td><a class="dl" href="/d/'+enc+'">Download</a></td></tr>';
    }).join('');
  ...
  function filter(){
    ...
    render(all.filter(x=>x.fname.toLowerCase().includes(q)));
  }
  render(all);
</script>
</body>
...
async function listPage(env) {
  ...
  let items = (listed.objects || []).map((o) => ({
    key: o.key,
    fname: o.key.split("/").pop(),
    size: fmtSize(o.size || 0),
    mod: o.uploaded ? new Date(o.uploaded).toISOString().slice(0, 10) : "",
  }));
