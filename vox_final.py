#!/usr/bin/env python3
"""
Vox-Style Video Creator — Uses curl for downloads
"""
...
def generate_images(storyboard):
    """Generate images using curl."""
    print(f"\n[2/4] Generating images...")
    
    images = []
    for scene in storyboard["scenes"]:
        prompt = scene["visual_description"].replace(" ", "%20")
        url = f"https://image.pollinations.ai/prompt/{prompt}?width=1920&height=1080&nologo=true&seed={scene['scene_number']}"
        
        filepath = TEMP_DIR / f"scene_{scene['scene_number']:02d}.png"
        
        print(f"  Generating scene {scene['scene_number']}...")
        cmd = ["curl", "-s", "-L", "-o", str(filepath), url]
...
def create_videos(images):
    """Create videos with simple zoom effect."""
    print(f"\n[3/4] Creating videos...")
    ...
    for i, img_path in enumerate(images, 1):
        ...
        cmd = [
            ...
            "-loop", "1",
            "-i", str(img_path),
            "-vf", "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,zoompan=z='min(zoom+0.0015,1.2)':d=125:s=1920x1080:fps=25",
            "-c:v", "libx264",
            "-t", "5",
            "-pix_fmt", "yuv420p",
            str(video_path)
